...
def extract_code_from_redirect_url(redirect_url: str, expected_state: str) -> str:
    ...
    if error:
        ...
        raise RuntimeError(
            f"OAuth2 authorization failed: {error} {description}".strip()
        )
...
def _read_legacy_oauth2_values(config_env_path: Path) -> dict[str, str | None]:
    ...
    return {
        key: config_values.get(key) for key in OAUTH2_ENV_KEYS if key in config_values
    }
...
def _parse_token_response(resp: httpx.Response) -> dict[str, Any]:
    ...
    if not resp.is_success:
        ...
        raise RuntimeError(
            f"OAuth2 token request failed (HTTP {resp.status_code}): {msg}"
        )
